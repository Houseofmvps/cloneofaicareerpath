...
export default function AutoApplyPage() {
  ...
  const saveNotificationPrefs = async (newPrefs) => {
    try {
      await axios.post(`${API}/notifications/preferences`, newPrefs, {
        headers: { Authorization: `Bearer ${localStorage.getItem("token")}` }
      });
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-purple-500/10 border border-purple-500/30 mb-4">
                ...
                <span className="text-sm text-purple-300">AI Auto-Apply</span>
                <span className="text-xs bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded-full ml-2">
                  BETA
                </span>
              </div>
              <h1 className="text-3xl font-bold">Apply to Jobs While You Sleep</h1>
              <p className="text-muted-foreground">
                Set your preferences once. We scan job boards and auto-apply for you.
              </p>
            </div>
            ...
            <div className="glass rounded-xl p-4 text-center">
              <button
                onClick={() => {
                  setPreferences(prev => ({
                    ...prev,
                    auto_apply_enabled: !prev.auto_apply_enabled
                  }));
                  savePreferences();
                }}
              ...
              <div className="text-sm mt-2">
                {preferences.auto_apply_enabled ? (
                  <span className="text-emerald-400">Auto-Apply ON</span>
                ) : (
                  <span className="text-muted-foreground">Auto-Apply OFF</span>
                )}
              </div>
            </div>
          </div>
